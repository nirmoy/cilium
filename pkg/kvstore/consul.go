...
func (c *consulModule) setConfigDummy() {
	...
	if err != nil {
		log.WithError(err).Warnf("unable to read consul configuration file %s", consulDummyConfigFile)
	}

	err = yaml.Unmarshal(b, &yc)
	if err != nil {
		log.WithError(err).Warnf("invalid consul configuration in %s", consulDummyConfigFile)
	}
...
func (c *consulModule) newClient() (BackendOperations, error) {
	if c.config == nil {
		...
		if configPathOptSet && configPathOpt.value != "" {
			...
			if err != nil {
				return nil, fmt.Errorf("unable to read consul configuration file %s: %s", configPathOpt.value, err)
			}
			yc := consulAPI.TLSConfig{}
			err = yaml.Unmarshal(b, &yc)
			if err != nil {
				return nil, fmt.Errorf("invalid consul configuration in %s: %s", configPathOpt.value, err)
			}
			c.config.TLSConfig = yc
		}
